...
>**关于实时变声**
>
>开发者花了很长时间来测试实时变声，但就是不成功，如果你有什么建议，可以到issue提出，我会非常感谢的！
...
> **如果你在使用A2后验流训练模型的话......下面是我的一些建议**
> 
> ***
> 
> 首先，A2后验流**没有**类似于Sovits的G_0和D_0作为**底模**，在常规情况下会直接**随机**开始整个训练。
> 
> 这就是为什么A2后验流需要更多步数才能完成模型的训练，且这样的训练很难把握好尺寸（少了模型学不完，多了会有过拟合的风险）。
> 
> 所以我的建议是可以先训练一个底模，这个底模的训练集可以有**3-5个人**的讲话素材，数据集长度在**45分钟-120分钟**，然后训练15000step差不多。
> 
> 底模的训练完成后，**找到G_15000.pth和D_15000.pth这两个文件**，然后把这两个文件重命名为G_0_rvc.pth和D_0_rvc.pth，通过**管理->预训练->其他文件**上传到服务器就可以了。
> 
> 这样下来训练的时候就会自动调用这两个文件作为底模，从而加快训练进度。
> 
> 如果你有更好的方法，欢迎一起讨论！
>
...
> 大文件建议 scp 直接放到 `pretrain/` 目录或者通过`管理->预训练`放到服务器，启动后会自动识别。
