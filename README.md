...
- `hybrid`（推荐）：NF 逆变换给出起点，FM 用少量步数（默认 4）精修，质量接近 FM、速度接近 NF

FM 训练时用 NF 输出作为起点（保证训练/推理一致性），FM 输出头零初始化（初期 Hybrid ≈ NF，渐进学习速度场）。完整使用说明见 [docs/unified_flow.md](docs/unified_flow.md)。
