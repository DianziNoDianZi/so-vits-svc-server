    ...
    for k, v in state_dict.items():
        try:
            ...
            if "enc_q" not in k or "emb_g" not in k:
              ...
              new_state_dict[k] = v
    if mismatch_count > 3:
        logger.info(f"... and {mismatch_count - 3} more layers initialized from scratch (encoder change detected)")
    if hasattr(model, 'module'):
...
def save_checkpoint(model, optimizer, learning_rate, iteration, checkpoint_path):
  ...
  if basename in ('G_0.pth', 'D_0.pth'):
    logger.warning(f"Skipping save to {basename} - would overwrite base model!")
    return
